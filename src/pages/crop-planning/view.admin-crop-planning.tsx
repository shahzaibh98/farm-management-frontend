...
const ManageCropsPlanning = () => {
  ...
  const handleAddFarmAdmin = () => {
    navigate(
      roleId === '0' ? '/admin-crops-planning/add' : '/manage-users/add'
    );
  };
  ...
  return (
    <main className={`w-full h-screen relative bg-darkColors-700`}>
      ...
      <GenericHeader
        headerText={roleId === '0' ? 'Crops Planning' : 'User'}
        breadcrumbsText={`Manage ${roleId === '0' ? 'Crops Planning' : 'User'}`}
        isAddOrUpdateButton
        buttonContent={`Add ${roleId === '0' ? 'Crop Planning' : 'User'}`}
